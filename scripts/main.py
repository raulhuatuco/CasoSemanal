...
from manttos import procesar_mantenimientos
...
for datos in RutasExcelDatos:
    ...
    '''
    Procesa los mantenimientos y convierte su formato al mismo de
    ...
    df_mantos = procesar_mantenimientos(df_mantos,
                                        df_centrales,
    ...
    #cambio de formato
    df_mantos["inicio"] = df_mantos["inicio"].clip(lower=nueva_fecha_base)
    df_mantos["final"]  = df_mantos["final"].clip(lower=fecha_fin)
    
    df_mantos = pd.concat([df_mantos.reset_index(drop=True),
                           df_codigos.loc[df_mantos.loc[:,"central/modo"]].reset_index(drop=True)
                           ],
                          axis=1
                          )
    ...
    df_mantos_yupana = cambiar_formato_diario(df_datos=df_mantos,
    ...
    df_mantos_yupana["Restriccion"] = np.where(df_mantos_yupana["Categoria equipo"] == 4,
                                               1,
                                               14
                                               )
    df_mantos_yupana["Desc. Restriccion"] = np.where(df_mantos_yupana["Categoria equipo"] == 4,
                                                     "Mantenimiento Planta Hidro",
                                                     "Mantenimiento Plantas Termicas"
                                                     )

    df_mantos_yupana = df_mantos_yupana.loc[:,df_DatRes.columns]
    df_mantos_yupana = df_mantos_yupana.sort_values(by=["Equipo","Desc. Restriccion"])
    ...
    df_DatRes = pd.concat([df_mantos_yupana,df_DatRes])
    
    del df_mantos_yupana,mask
